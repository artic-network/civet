...
def make_background_map_json(config): 
    ...
    with open(config["background_csv"]) as f: #this takes about 30 seconds for 1m sequences in Jupyter. It may require optimisation at some point
        data = csv.DictReader(f)
        for l in data:
            if l[geog_col] != "" and l[lin_col] != "":
                #date restriction in here
